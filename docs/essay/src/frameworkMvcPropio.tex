...
\section{Localización de la interfaz\label{extra:mvc:localizacion}}

gettext, php, necesario, js.php
...
Las variables que puede cambiar el usuario (idioma por defecto, tema visual por defecto y dirección IP del \gls{servicioweb} \gls{FPGA}) no se inicializan por definición en el código sino que se leen del fichero \texttt{./config/settings.json}. Además, los idiomas disponibles se extraen del archivo \texttt{./config/languages.json} y los temas visuales disponibles se deducen de los ficheros de la carpeta \texttt{./public/themes/}.
