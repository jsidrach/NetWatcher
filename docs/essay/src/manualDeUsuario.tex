...
En las siguientes subsecciones se explica como utilizar las principales pantallas interactivas de la interfaz web.
...
\end{itemize}
Para elegir un modo basta con pulsar uno de los dos botones de la interfaz (ver Figura \ref{fig:captura:gestionseleccion}). Esta elección de modo no es definitiva, ya que se puede cambiar de modo posteriormente siempre que la \gls{FPGA} no esté capturando o reproduciendo tráfico.
...
Una vez seleccionado un modo, un diálogo muestra el progreso de la inicialización: programando la \gls{FPGA}, reiniciando el servidor y montando la \gls{FPGA} (ver Figura \ref{fig:captura:gestionprogreso}).
...
\end{itemize}
Los dos primeros campos se iluminarán en verde cuando sean introducidos correctamente y en rojo cuando sean incorrectos. Cuando todos los campos sean válidos se activará el botón de \textit{Empezar}, y si lo pulsamos la \gls{FPGA} comenzará a capturar tráfico con los parámetros indicados.
...
Si la \gls{FPGA} ha sido inicializada en modo reproductor, la pantalla de gestión mostrará una tabla y un formulario (ver Figura \ref{fig:captura:gestionreproducir}). La tabla contiene una fila por cada \gls{traza} disponible, con su nombre, tipo (\gls{simple}), tamaño y fecha. Pulsando sobre una fila se seleccionará la \gls{traza} correspondiente para su reproducción. El formulario contiene los siguientes campos:
\begin{itemize}
...
\end{itemize}
Cuando se haya seleccionado una traza de la tabla y todos los campos del formulario sean válidos se activará el botón de \textit{Empezar}, y si lo pulsamos la \gls{FPGA} comenzará a reproducir la \gls{traza} seleccionada con los parámetros indicados.
...
TODO: Renombrar, convertir <->, borrar, Figura \ref{fig:captura:capturas}, referencia a la sección de configuración
\begin{figure}[H]
  \centering
