...
En el Código \ref{code:ejemplogettext} se describe un ejemplo de cómo imprimir una cadena traducida a otro idioma mediante una llamada a la función ``\textit{\_}''. Si el idioma base de los catálogos es el inglés y el idioma establecido en la aplicación es español, la función ``\textit{\_}'' busca la cadena en inglés (idioma base) \textit{`Example of string'} en el catálogo español y devuelve la cadena de texto \textit{`Ejemplo de cadena'}, que finalmente se imprime con \textit{echo}.
